...
class sideMenu extends HTMLElement {
  ...
  connectedCallback() {
    ...
    <aside class="h-full w-full">
      <div class="h-full flex ">
        <button class="open-sidebar btn btn-circle bg-white self-center ml-2 text-primary hover:text-primary-focus focus:outline-none focus:ring-2 focus:ring-white">
  ...
  async showFile(event) {
    ...
    contentBar.classList = "w-fit h-fit absolute right-0 inset-y-1/2"
    ...
    let excludes = ["pages", "siteMap", "meta", "alternatives", "points", "matterport", "locations", "interactables", "rotation", "defaultPage"]
      for (const [key, name, value] of this.loop(content, excludes, prefix)) {
        ...
        if(key === "activeColor"){
          ...
          element.dataset.name = name;
          element.dirHandle = this.dirHandle;
          output.push(element);
        }
